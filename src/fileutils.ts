...
async function getOrCreateCompressedAlbum(): Promise<string> {
  const albumName = 'Compressed Images';
  const existing = await Media.getAlbums();
  const album = existing.albums.find((item) => item.name === albumName);
  if (album) return album.identifier;

  await Media.createAlbum({ name: albumName });
  const updated = await Media.getAlbums();
  const created = updated.albums.find((item) => item.name === albumName);
  if (!created) throw new Error('Unable to create Compressed Images album');
  return created.identifier;
}

async function shareNativeFile(blob: Blob, fileName: string): Promise<string> {
...
export async function downloadImageToDevice(input: string | Blob, fileName: string) {
  ...
  // 1. Native Android gallery save. This path is used only inside the
  // Capacitor native app, so browsers never depend on the native plugin.
  if (Capacitor.getPlatform() === 'android') {
    try {
      const albumIdentifier = await getOrCreateCompressedAlbum();
    ...
    // 2. Native Android share/save sheet fallback.
    try {
      return await shareNativeFile(blob, fileName);
    } catch (shareErr) {
      console.warn('Native share failed, using web fallbacks', shareErr);
    }
  }

  // 3. Mobile browser Web Share fallback.
  try {
    const file = new File([blob], fileName, { type: blob.type || 'image/jpeg' });
    if (
      typeof navigator !== 'undefined' &&
    ...
    console.warn('Web Share unavailable or dismissed', err);
  }
